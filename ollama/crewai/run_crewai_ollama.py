"""
CrewAI multi-agent driver evaluation with local Ollama (OpenAI compatible).

Agents:
- Drafter: produces initial report.
- Reviewer: audits structure/actions and returns notes.
- Finalizer: outputs the corrected final report.
...
def ollama_chat(prompt: str, images: List[Path], max_tokens: int = 900, temperature: float = 0.3) -> str:
    if _CLIENT is None:
...
@tool("draft_report", result_as_answer=True)
def draft_report() -> str:
    """Generate the initial 5-section driving report from the template prompt."""
    return ollama_chat(_PROMPT, _FRAMES, max_tokens=900, temperature=0.3)


@tool("review_report", result_as_answer=True)
def review_report(draft: str) -> str:
    """Review the report and return concise correction notes (no rewrite)."""
    action_set = ""
    if "Action set:" not in draft:
        action_set = ""
    prompt = (
    ...
    return ollama_chat(prompt, _FRAMES, max_tokens=400, temperature=0.2)


@tool("final_report", result_as_answer=True)
def final_report(draft: str, review: str) -> str:
    """Produce the final report using draft + reviewer notes, keeping strict template format."""
    prompt = (
    ...
    return ollama_chat(prompt, _FRAMES, max_tokens=900, temperature=0.3)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CrewAI multi-agent driver evaluation with local Ollama.")
...
def main() -> None:
    global _CLIENT, _MODEL, _FRAMES, _PROMPT
    args = parse_args()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        _PROMPT = build_prompt(payload)
        _FRAMES = sample_frames(video_path, tmp_dir, args.num_frames)
        if not _FRAMES:
            raise RuntimeError("No frames sampled; check video or imageio[pyav].")

        _MODEL = args.model
        _CLIENT = OpenAI(api_key="ollama", base_url=args.base_url)

        # Direct pipeline (no CrewAI LLM planning) to avoid Ollama timeouts.
        draft = draft_report.run()
        review = review_report.run(draft)
        final_text = final_report.run(draft, review)

        if args.output:
            out_path = Path(args.output)
            out_path.parent.mkdir(parents=True, exist_ok=True)
            out_path.write_text(str(final_text).strip(), encoding="utf-8")
        else:
            print(str(final_text).strip())
