...
if __name__ == "__main__":
    ...
    if args.policy_model == "rt1":
        ...
        assert args.ckpt_path is not None
        from simpler_env.policies.openvla.openvla_oft_model import OpenVLAOFTInference
        model = OpenVLAOFTInference(
            saved_model_path=args.ckpt_path,
            policy_setup=args.policy_setup,
            action_scale=args.action_scale,
            unnorm_key="example_dataset",  # Use the same unnorm_key as regular OpenVLA
            use_diffusion=True,  # Using diffusion head as specified
            num_diffusion_steps=50,
            use_l1_regression=False,
            use_proprio=False,
            center_crop=False,  # No center crop - direct resize
            load_in_8bit=False,
            load_in_4bit=False,
