...
from models.qwen2_5_vl.configuration_qwen2_5_vl_memory import Qwen2_5_VLConfig, Qwen2_5_VLVisionConfig
import os
...
from models.modeling_beacon import Memory
from models.modeling_utils import optional_grad_ctx, compute_loss, BeaconModelOutput
...
class Qwen2_5_VisionPatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = hidden_states.to(ori_dtye)
...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, x, beacon_size = 0, beacon_indices = None):
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        use_cache = True  # BEACON: always use cache
        ...
        if cache_position is None:
            ...
            past_seen_tokens = past_key.shape[2] if past_key is not None else 0
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def get_rope_index(
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                # print(f'pos_ids: {position_ids.shape}, llm_pos: {llm_positions.shape}  mask.shape: {(attention_mask[i]==1).shape}')
                position_ids[..., i, attention_mask[i] == 1] = llm_positions.to(position_ids.device)
                mrope_position_deltas.append(llm_positions.max() + 1 - len(total_input_ids[i]))
            # print('delats: ', mrope_position_deltas)
    ...
    def forward(
        ...
        # print(f'pos: ',position_ids,'  inputs_embeds: ',inputs_embeds.shape,' cache_pos: ',cache_position)
        # self.generate()
        outputs = self.model(
            input_ids=None,
            position_ids=position_ids,
        ...
        ### save attn weights ###
        # attns = outputs[-1]
        # print('output-atten: ',len(outputs[-1]),' shape: ',outputs[-1][0].shape)
        # vis_dir = 'vis/memory'
        # os.makedirs(vis_dir,exist_ok=True)
        # past_key, past_value, beacon_size, beacon_indices = past_key_values[0]
        # if beacon_size == 0: # generation
        #     tokens = os.listdir(vis_dir)
        #     tokens = [token for token in tokens if 'token' in token]
        #     token_dir = join(vis_dir,'token_' + str(len(tokens)).zfill(3))
        #     os.makedirs(token_dir,exist_ok=True)
        #     file = f'layer_27.npy'
        #     np.save(join(token_dir,file),attns[-1].to(torch.float32).cpu().data.numpy())

        # else: # shot data
        #     shots = os.listdir(vis_dir)
        #     shots = [shot for shot in shots if 'shot' in shot]
        #     shot_dir = join(vis_dir,'shot_' + str(len(shots)).zfill(3))
        #     os.makedirs(shot_dir,exist_ok=True)
        #     file = f'layer_27.npy'
        #     np.save(join(shot_dir,file),attns[-1].to(torch.float32).cpu().data.numpy())
