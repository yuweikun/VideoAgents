...
class Memory(torch.nn.Module):
    ...
    def prepare(self, input_ids, attention_mask, labels, split_indices, skip_first=None, skip_last=None):
        """
        ...
        # ### NOTE: reset start_idx, end_idx, step_idx, activations, etc. before every forward.
        # self.start_idx = 0
        # self.end_idx = 0
        # self.step_idx = 0
        # self.raw_activations = [(None, None) for _ in range(self.config.num_hidden_layers)]
    ...
    def _step(self, ignore_memory=False):
        ...
        #     start_idx = 0
        # else:
        #     start_idx = self.split_indices[self.shot_idx-1] + 1
        # print('step-idx: ',self.step_idx,' start_ids: ',self.start_idx,'  end-idx: ',self.end_idx)
        start_idx = self.start_idx
        self.beacon_window = self.split_indices[self.step_idx] - self.start_idx
        self.beacon_stride = self.beacon_window
        # the end position of the current window w.r.t. the start of the current input sequence
        end_idx = start_idx + self.beacon_window
        # if self.is_last_shot:
        #     end_idx = self.all_sequence_length
        # else:
        #     end_idx = self.split_indices[self.shot_idx+1]
        ...
        # print(f'len(split_indices): {len(self.split_indices)} step_idx: {self.step_idx} start_idx: {start_idx}  end_idx: {end_idx}  is_full_window: {is_full_window}')
        ...
        # print(f'start_idx: {start_idx} end_idx: {end_idx}, beacon size: {beacon_size} is_full_window: {is_full_window}')
        ...
        # print('memory, mask: ',attention_mask.shape,'  pos: ',position_ids.shape)
        ...
        # return input_ids, attention_mask, position_ids, past_key_values, labels
        return input_ids, past_key_values, attention_mask, position_ids, labels
    ...
    def update_loss(self, batch_loss, valid_token_num):
        ...
        """
        # print(f'batch_loss: ',batch_loss,' valid token num: ',valid_token_num)
