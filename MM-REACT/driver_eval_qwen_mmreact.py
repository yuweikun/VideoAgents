...
def qwen_chat(prompt: str, video_uri: str, max_tokens: int = 900, temperature: float = 0.2) -> str:
...
def run_once(video_path: Path, payload: Dict[str, Any]) -> Dict[str, Any]:
    prompt = build_prompt(payload)
    video_uri = str(video_path.resolve())
    draft = qwen_chat(prompt, video_uri=video_uri)
    return {"video_uri": video_uri, "payload": payload, "draft_report": draft}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Driver evaluation via Qwen3-VL (single pass, MM-REACT adaptation).")
