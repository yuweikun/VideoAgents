...
for p in (ROOT, PKG_ROOT, ROOT / "ReAgent-V", ROOT / "ReAgent-V" / "ReAgent-V", ROOT / "ReAgent-V" / "ReAgent-V" / "ReAgentV_utils"):
    if str(p) not in os.sys.path:
        os.sys.path.append(str(p))

from template import INSTRUCTION, one_shot  # type: ignore
from ReAgentV_utils.prompt_builder.prompt import eval_reward_prompt_template  # type: ignore
